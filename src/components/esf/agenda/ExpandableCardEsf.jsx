...
const ExpandableCardEsf = ({ dayTitle, date, summary, location, agendaList }) => {
    ...
    return (
        <div
            className="w-full max-w-4xl mx-auto my-6 bg-[#EAF4E6] text-[#1D6935] rounded-3xl shadow-md hover:shadow-lg cursor-pointer transition-all duration-300"
            onClick={() => setIsExpanded(!isExpanded)}
        >
            <div className="p-6 md:p-8">
                <div className="relative flex justify-center items-center">
                    <h2 className="text-3xl md:text-4xl font-bold">{dayTitle}</h2>
                    <span
                        className={`absolute right-4 text-[#1D6935] text-3xl transition-transform duration-300 ${isExpanded ? "rotate-45" : ""}`}
                    >
                        +
                    </span>
                </div>

                <div className="mt-2 text-base md:text-lg text-center space-y-1">
                ...
                {isExpanded && (
                    <div className="mt-6 bg-[#FBFCF7] p-6 md:p-8 rounded-3xl shadow-inner transition-all duration-300 overflow-x-auto">
